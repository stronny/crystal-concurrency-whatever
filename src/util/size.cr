struct XNT::Size
  ...
  module Error
    class Overflow < Exception
      def initialize(cur : Type, dx : Type, max : Type)
        super "New size will overflow, current: #{cur}, increase: #{dx}, max: #{max}"
      end
    end

    class Underflow < Exception
      def initialize(cur : Type, dx : Type, min : Type)
        super "New size will underflow, current: #{cur}, decrease: #{dx}, min: #{min}"
      end
    end
  end
