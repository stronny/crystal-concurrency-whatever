class XNT::Jibers
  ...
  def self.make_jiber_running(jiber : XNT::Jiber) : Nil
    status = @@table.delete jiber.id do |k|
      raise Error::NotFound.new k
    end
  ...
  def self.wake_me_up_after_all_of_these_children_start(jids : Indexable(XNT::Jiber::ID)) : Nil
    ...
    select_created jids do |jid|
      @@wakeup_table_every[Fiber.current][jid] = nothing
    end
